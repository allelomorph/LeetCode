...
// Time to complete: 3 hrs (includes failed attempts)

// Needed to research:

// Note: accepted, but terrible performance in both speed and space - can
//   can recursion be improved by passing a single vector and modifying it?

//// Begin solution, results at https://leetcode.com/submissions/detail/743999743/
...
public:
    ...
    std::vector<std::vector<int>> combine(const int n, const int k) {
        ...
        for (int i {1}; i <= n; ++i) {
            recursiveCombination(combos, std::vector<int> {}, 0, i, k, n);
        }
        return combos;
    }
private:
    void recursiveCombination(std::vector<std::vector<int>> &combos,
        ...
        combo.push_back(combo_elem);
        for (int i {combo.back() + 1}; i <= n; ++i)
